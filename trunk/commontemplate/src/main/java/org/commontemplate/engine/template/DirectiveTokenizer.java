...
final class DirectiveTokenizer {
	...
	private static final int states[][] = { // TODO ":"语法待删除
		...
		/* 2.指令    */ { 2, R, R, 3, 18, 5, R, 6, 7, 8, R, R, R, R, R}, // 指令前导符识别
		/* 3.名称    */ { 4, R, B, 3, 18, 5, B, B, B, B, B, B, B, B, B}, // 读取指令名
		/* 4.中空    */ { 4, R, L, L, 18, 5, L, L, L, L, L, L, L, L, L}, // 退还没有表达式的指令名后的空白符, 处理场景: $end <空格>
		/* 5.表达式  */ { 5, 5, 5, 5, 5, R, E, 5, 5, 5, 5, 12, 14, 16, 5}, // 表达式大括号识别
		/* 6.不解释块*/ { 6, 19, 6, 6, 6, 6, 6, 10, 6, 6, 6, 6, 6, 6, 6}, // 不解释块特殊指令识别
		/* 7.块注释  */ { 7, 20, 7, 7, 7, 7, 7, 7, 11, 7, 7, 7, 7, 7, 7}, // 块注释特殊指令识别
		/* 8.行注释  */ { 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, N, 8, 8, 8, 8}, // 行注释特殊指令识别, 注释不包含换行符
		/* 9.文本    */ { 9, 1, B, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9}, // 文本表示
		/*10.结束6  */ { 6, 6, E, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6}, // 结束不解释块
		/*11.结束7  */ { 7, 7, E, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7}, // 结束块注释
		/*12.引号    */ { 12, 13, 12, 12, 12, 12, 12, 12, 12, 12, 12, 5, 12, 12, 12}, // 绕过表达式中的双引号字符串(不解析字符串中的内容)
		/*13.转义    */ { 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12}, // 双引号字符串转义
		/*14.单引号  */ { 14, 15, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 5, 14, 14}, // 绕过表达式中的单引号字符串(不解析字符串中的内容)
		/*15.转义    */ { 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14}, // 单引号字符串转义
		/*16.反单引号*/ { 16, 17, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 5, 16}, // 绕过表达式中的反单引号字符串(不解析字符串中的内容)
		/*17.转义    */ { 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16}, // 反单引号字符串转义
		/*18.分隔    */ { 18, B, B, 18, B, B, B, B, B, B, B, B, B, B, B}, // 简化语法, 冒号分隔符
		/*19.转义6  */ { 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6}, // 块注释结束符转义, 如: $* \*$ *$
		/*20.转义7  */ { 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7} // 不解释块结束符转义, 如: $! \!$ !$
	};
