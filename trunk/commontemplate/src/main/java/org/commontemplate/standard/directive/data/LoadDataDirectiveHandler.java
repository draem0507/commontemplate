...
public class LoadDataDirectiveHandler extends DirectiveHandlerSupport {
	...
	public void doRender(Context context, String directiveName, Object param) throws Exception {
		if (param instanceof String) {
			...
			loadData(context, path.substring(i + 1), path);
		} else if (param instanceof Entry) {
			Entry entry = (Entry)param;
			loadData(context, String.valueOf(entry.getKey()), String.valueOf(entry.getValue()));
		} else {
			Assert.fail("$load指令参数错误!"); // TODO 未国际化
		}
	}

	private void loadData(Context context, String type, String path) throws Exception {
		...
		Resource resource = context.loadResource(path);
