...
import org.commontemplate.ext.coat.common.Attribute;
...
public class JerichoElement implements TagElement{
	...
	public Attribute[] getAttributes() {
		if(attributes==null){
			...
			for(Iterator it = attrs.iterator();it.hasNext();i++){
				au.id.jericho.lib.html.Attribute attr = (au.id.jericho.lib.html.Attribute) it.next();
	...
	public Segment[] getSegments() {
		if(segments == null){
			...
			segments[c] = new TextSegment((String) source.subSequence(pos, element.getEnd()));
		}
		return segments;
	}
