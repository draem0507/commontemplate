<!--$extends{"/doc/template/frame.ctl"}-->
	<!--$overzone{"content"}-->
								...
								7. 人性化的标准指令集设计，更符合常规思维(最少惊讶原则)，展现力更强。<br/>
								8. 支持宏引用及模板继承，更有利于模板组件化，尤其是模板继承，采用面向对象的多态思想，比宏替换更灵活。<a href="demo_extends.html">示例...</a><br/>
								9. 内置的国际化支持，国际化信息处理更方便。<br/>
								10. 完备的表达式支持，在全面兼容Java表达式的基础上，增加了更富表达力的操作符，包括lambda表达式等。<br/>
								11. 禁止void函数调用，避免在模板中引入业务逻辑。(根据契约式设计原则，void函数通常是有副作用的，即会修改状态)<br/>
								12. 对已有的不可变类(String,Number,Date等)，采用open class思想，允许在类的外部给类添加新的属性或方法，如：String本没有“首字母大写”的功能，可以外部给String注册一个capital属性：\${"james".capital}。(类似javascript的prototype)<br/>
								13. 引擎采用微核设计理念，除了核心API及解析器外，其它如语言定义，资源管理等都是外置的，可以基于同一个引擎，设计另一套完全不同的模板语言。<br/>
								14. 高度可扩展，平等对待标准包与第三方扩展，标准包所能实现的任何功能均可被替换。<a href="extension.html">更多...</a><br/>
								15. 使用主控迭代器模式替代传统的被动访问器模式,引擎只负责将模板解析成指令树，其它所有处理均由扩展指令自身完成，给扩展指令以最大的控制权。(类似于StAX与SAX的区别)<br/>
								16. 多种优化措施，性能更优：<br/>
								...
								17. 站在巨人的肩膀上，参考并借鉴了 WebMacro, Velocity, FreeMarker, JavaFX, OGNL, Perl6, Python/Django, PHP/Smarty, JavaScript/JSON, XML/XPath 等开源项目或规范，感谢它们做出的贡献。<br/>
								<br/>
