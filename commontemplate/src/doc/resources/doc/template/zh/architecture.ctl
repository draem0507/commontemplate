...
$!
								...
								<b>二. 类结构设计说明</b><br/>
								<b>类设计原则：</b><br/>
								开闭原则(OCP)：对扩展开放，对修改关闭，尽可能保证所有类在写好后就不再改动，这是最基本的原则，也是设计目标之一。<br/>
								里氏代换原则(LSP)：子类应该在任何情况都能替换父类（包括逻辑上的）。<br/>
								单一职责原则(SRP)：每个类只封装一个变化因子，多个变化因子时可考虑使用桥模式等组合式拆分职责。<br/>
								接口隔离原则(ISP)：依赖抽象（接口），隐藏具体（实现），由其是服务域的API，对外的门面必须稳定。<br/>
								依赖倒置原则(IoC/DI)：不主动依赖实现类或工厂，只声明依赖接口，由框架容器等外部服务注入依赖实例并管理其生命周期。<br/>
								合成/聚合复用原则(CARP)：使用合成或聚合的方式，而不是继承的方式，进行代码复用。<br/>
								...
								(1) 标签框表示各领域模型范围<br/>
								(2) 最左边一列各引擎所跨行，表示这些行上的接口或抽象类由该引擎所实现<br/>
								(3) 类颜色采用UML四色原型[COAD95-97]进行标识：<br/>
								...
								(1) 标签框表示配置分类<br/>
								(2) 最左边一列各引擎所跨行，表示这些行上的配置类用于向该引擎提供配置信息<br/>
								(3) 类颜色标识参见核心包<br/>
!$
	<!--$end-->
<!--$end-->